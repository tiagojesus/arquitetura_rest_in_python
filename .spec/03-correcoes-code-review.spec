...
- [x] **T01 — Corrigir `app/db.py`** (resolve C1)
...
- [x] **T02 — Reescrever `app/services/exceptions.py`** (resolve C2)
...
- [x] **T03 — Credenciais do admin via Settings** (resolve C3)
...
- [x] **T06 — Resolver passlib × bcrypt** (resolve M4)
...
- [x] **T07 — Zerar o ruff** (resolve m1, m2)
...
- [x] **T08 — Robustecer `tests/test_docs.py`** (resolve m3)
...
- [x] **T09 — Atualizar a spec 02** (resolve m4 + alinhamento RN02/RN06)
...
- [x] Nenhum arquivo contém blocos duplicados (db.py, config.py,
  exceptions.py, models/__init__.py).
- [x] Seed executa no startup real: banco recriado do zero contém o admin.
- [x] Nenhuma credencial hard-coded no código-fonte.
- [x] `docker compose run --rm tests` passa com todos os testes
  (existentes + ajustados).
- [x] `ruff check .` retorna 0 erros.
- [x] Contrato externo da API inalterado.
